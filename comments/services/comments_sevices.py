...
from social_networks.services.services import redirect_to_entry_page
...
def create_comment_for_specific_entry(request, entry_id):
    ...
    if request.method == "POST":
        ...
        if comment_form.is_valid():
            return _add_comment_to_mongodb_and_return_json_for_frontend(
                    request, entry_id, comment_form)        

    return redirect_to_entry_page(entry_id)


def _add_comment_to_mongodb_and_return_json_for_frontend(
    request, entry_id, comment_form):
    """
    Add comment to monogdb and return Json for dynamic changing on frontend.
    """

    comment_for_mongodb = _get_comment_format_for_mongodb(
        request, entry_id, comment_form)
    ...
    if request.is_ajax():
        json_for_frontend = _send_json_response_on_create_comment_for_frontend(
                                                            comment_for_mongodb)
...
def delete_main_comment_and_its_subcomment(request, entry_id, main_comment_id):
    ...
    return _delete_comment_if_user_is_author(
        topic_id, entry_id, main_comment_id, current_username)


def _delete_comment_if_user_is_author(
    topic_id, entry_id, main_comment_id, current_username):
    """
    ...
    comment = comments_collection.find_one({
        ...
        "comment_author": current_username
        })
...
def add_subcomment_for_specific_comment(
    request, entry_id, main_comment_id, comment_id_for_subcomment):
    """
    Add subcomment for specific comment. 
    Can add subcomment for other subcomment.
    """
    ...
    if form.is_valid():
        ...
        main_comment = get_comments_collection().find_one(
            {"_id": ObjectId(main_comment_id)}
            )

        if 'subcomments' in main_comment:
            subcomments = main_comment['subcomments']

            _add_subcomment_for_main_comment_or_other_subcomment(
                subcomments, main_comment_id, comment_id_for_subcomment,
                comment_author, comment_text, datetime_now)
            
        else:
            if _is_subcomment_for_main_comment(
                main_comment_id, comment_id_for_subcomment):

                _add_first_subcomment_for_main_comment(
                    main_comment_id, comment_author, 
                    comment_text, datetime_now)

    return redirect_to_entry_page(entry_id)


def _is_subcomment_for_main_comment(main_comment_id, 
                                    comment_id_for_subcomment):
    """
...
def _add_first_subcomment_for_main_comment(
    comment_id, comment_author, comment_text, datetime_now):
    """
    ...
    get_comments_collection().update(
        ...
        {"$set": {"subcomments": [first_subcomment]}}
        )


def _add_subcomment_for_main_comment_or_other_subcomment(
    subcomments, main_comment_id, comment_id_for_subcomment,
    comment_author, comment_text, datetime_now):
    """
...
def _finds_needed_comment_and_add_subcomment_for_it(
    subcomments, comment_id_for_subcomment, comment_text,
    comment_author, datetime_now):
    """
    Recursion for finding needed subcomment and adding subcomment for it.
    Takes every subcomment in subcomments list
    Is checking is subcomment id from list of suequal id of needed subcomment.
    ...
    for subcomment in subcomments:
        if subcomment["id"] == ObjectId(comment_id_for_subcomment):
            ...
            if 'subcomments' in subcomments:
                existing_subcomments = subcomment["subcomments"]
                _finds_needed_comment_and_add_subcomment_for_it(
                    existing_subcomments, comment_id_for_subcomment,
                    comment_text, comment_author, datetime_now)

    return subcomments


def delete_subcomment_and_its_subcomments(
    request, entry_id, main_comment_id, subcomment_id):
    """
    Deletes specific subcomments and its subcomments if they are existing
    """

    comments_collection = get_comments_collection()
    main_comment_document = comments_collection.find_one(
        {"_id": ObjectId(main_comment_id)}
        )
    ...
    updated_subcomments = _find_and_delete_subcomment(
        subcomments, subcomment_id, username)
...
def _find_and_delete_subcomment(subcomments, subcomment_id, username):
    ...
    for subcomment in subcomments:
        if subcomment["id"] == ObjectId(subcomment_id):
            ...
            try:
                existing_subcomments = subcomment["subcomments"]
                _find_and_delete_subcomment(
                    existing_subcomments, subcomment_id, username)
            except KeyError:
                pass
        index += 1
